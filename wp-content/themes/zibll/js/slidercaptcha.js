 ...
 * @LastEditTime: 2020-12-31 23:11:46
...
function ajax_slidercaptcha(successFun, _this) {
    if ($(".slidercaptcha").length) {
        $('a[href="#tab-slidercaptcha"]').tab('show');
        var tab_id = false;
        if (_this) {
            _this.parents('.tab-pane').each(function (i) {
                if ($(this).siblings('.slidercaptcha-tab').length) {
                    tab_id = $(this).attr('id');
                }
            });
        }
        if (tab_id) {
            $('.slidercaptcha-back').attr('href', '#' + tab_id).fadeIn();
        }
        $('.slidercaptcha').sliderCaptcha({
            onSuccess: function (d) { //成功执行的函数
                _win.slidercaptcha = d;
                $.isFunction(successFun) && successFun(_this);
            },
            setSrc: function () { //图片获取函数
